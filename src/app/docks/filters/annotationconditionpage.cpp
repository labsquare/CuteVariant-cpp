...
FilterItem *AnnotationConditionPage::toItem() const
{

    return new FilterItem(mFields[mFieldbox->currentIndex()],
...
void AnnotationConditionPage::fromItem(FilterItem *item)
{
...
{
    mOperatorBox->clear();
    for (Operator::Type type : FilterFieldFactory::operatorsList(mFields[mFieldbox->currentIndex()]))
...
void AnnotationConditionPage::loadForm()
{
    ...
    if (mValueWidget == nullptr)
        mValueWidget = FilterFieldFactory::widget(mFields[mFieldbox->currentIndex()]);
